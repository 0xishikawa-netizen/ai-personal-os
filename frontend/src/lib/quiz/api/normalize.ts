...
export function normalizeQuestion(raw: Record<string, unknown>): QuizQuestion {
  ...
  return {
    ...
    tags: Array.isArray(raw.tags)
      ? (raw.tags as { id: string; name: string }[]).map((t) => ({ id: t.id, name: t.name }))
      : undefined,
    choices,
...
export function sectionFromApi(o: Record<string, unknown>): QuizSection {
  ...
  return {
    ...
    description: o.description != null ? String(o.description) : undefined,
    memo:
      memoRaw != null && String(memoRaw).trim() !== '' ? String(memoRaw) : undefined,
    order: Number(o.order) || 0,
  };
}

export function chapterFromApi(o: Record<string, unknown>): QuizChapter {
  return {
    id: String(o.id),
    sectionId: String(o.sectionId),
    title: String(o.title),
